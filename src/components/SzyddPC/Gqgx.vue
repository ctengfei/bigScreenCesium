<!--灌区供需-->
<template>
  <div class="gqgxpc_box">
    <div class="gqgxpc-skinfo">
      <ul class="gqgxpc-plan">
        <li class="gqgxpc-planItem" v-for="(item,index) in planInfoKey" :key="index" :style="{'width':item.width}">
          <span class="name">{{ item.titleName }}：</span>
          <span class="value">{{ currentPlan[item.key] || '--' }}{{ item.unit }}</span>
        </li>
      </ul>
      <div class="gqgxpcBtn" @click="handlePlan">切换方案</div>
    </div>
    <div style="display: flex;justify-content: space-between">
      <content-tab
          ...
          class="gqgxpc-tab"
      ></content-tab>
      <el-select v-show="tabIndex == 1" style="  margin-top: 0" v-model="sels" placeholder="请选择" class="custom-select">
        <el-option
    ...
    <!-- 灌溉量图图 -->
    <BuildEletricCharts ref="ddchart" :echartsData="{option:echartOption}" class="gqgxpcChart"/>
    <el-dialog
...
var colorList = ['#1791FF', '#FC7F00', '#67c23a', '#57C777']
var areaColorList = [
  new echarts.graphic.LinearGradient(0, 0, 0, 1, [
    {
      offset: 0,
      color: "rgba(23, 145, 255, 0.0)", // 0% 处的颜色
    },
    {
      offset: 1,
      color: "rgba(23, 145, 255, 0)", // 100% 处的颜色
    }
  ]),
  new echarts.graphic.LinearGradient(0, 0, 0, 1, [
    {
      offset: 0,
      color: "rgba(252, 127, 0, 0.0)", // 0% 处的颜色
    },
    {
      offset: 1,
      color: "rgba(252, 127, 0, 0.0)", // 100% 处的颜色
    }
  ]),
  new echarts.graphic.LinearGradient(0, 0, 0, 1, [
    {
      offset: 0,
      color: "rgba(103, 194, 58,0.0)", // 0% 处的颜色
    },
    {
      offset: 1,
  ...
  new echarts.graphic.LinearGradient(0, 0, 0, 1, [
    {
      offset: 0,
      color: "rgba(87, 199, 119, 0.0)", // 0% 处的颜色
    },
    {
      offset: 1,
      color: "rgba(87, 199, 119, 0)", // 100% 处的颜色
    }
  ])
]
export default {
    ...
    gqlineOption() {
      ...
      return {
        ...
        xAxis: {
          type: "category",
          boundaryGap: true,
          data: that.ggll_xAxisValues,//x轴数据
          axisLabel: {
            //坐标轴文字显示样式
            lineHeight: 15, //字体行高
            fontSize: 12,
    ...
    getggslEchart() {
      ...
      that.$api.szydd.getWirrStatistics(params).then((res) => {
        if (res.code == 200) {
          let list = res.data
          list.forEach(item => {
            let name = item.chanName
            let value = item.wirr
            that.ggl_Values.push(value)
            that.ggl_xAxisValues.push(name)
          })
        }
        that.echartOption = that.gqBarOption
      })
    },
    //获取灌溉流量数据
    getggllEchart(oss) {
          ...
          for (const resultKey in dates) {
            if (this.sels == resultKey) {
              ...
              dates[resultKey].forEach(r => {
                ...
                this.gqlineOption.series[0].name = r.chanName;
                r['qirr1'] = r.qirr // 补结尾
                that.ggll_series.push(r.qirr)
                that.ggll_series.push(r.qirr1)
              })
              that.ggll_xAxisValues = timeData;
            }
          }
        }
        that.echartOption = that.gqlineOption
      })
    },
  },
};
