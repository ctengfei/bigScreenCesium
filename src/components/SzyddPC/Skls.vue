<!--水库来水预测-->
<template>
  <div class="sklspc_box">
    <!-- <div class="sklspcBtn pcleft" :class="isShowLS?'pcchecked':''" @click="handleBtn(0)">来水预测</div>
    <div class="sklspcBtn pcright" :class="!isShowLS?'pcchecked':''" @click="handleBtn(1)">需水预测</div> -->
    <div class="sklspc-skinfo">
      <ul class="sklspc-plan">
        <li class="sklspc-planItem" v-for="(item,index) in planInfoKey" :key="index" >
          <span class="name">{{ item.titleName }}：</span>
          <span class="value">{{ currentPlan[item.key] || '--' }}</span>
        </li>
      </ul>
      <div class="sklspcBtn" @click="handlePlan">切换方案</div>
    </div>
    <content-tab :tabList="tabList" :tabIndex="tabIndex" @tabChange="handleTabChange"
                 class="sklspc_box-tab"></content-tab>
    <div class="sklspc-skls" v-show="isShowLS">
      <!-- <content-tab :tabList="tabList" :tabIndex="tabIndex" @tabChange="handleTabChange" class="sklspc_box-tab"></content-tab> -->
      <!-- 3d柱状图 -->
      <BuildEletricCharts :echartsData="buildEletric" class="sklspc_boxChart"/>
    </div>
    <skxs v-show="!isShowLS" :currentPlan="currentPlan" ref="skxs"></skxs>
    <el-dialog
  ...
  methods: {
    ...
    handleBtn(val) {
      if (val == 0) {
        ...
        this.isShowLS = false
        this.$refs.skxs.buildEchartSeries(that.currentPlan)
      }
    },
  }
}
