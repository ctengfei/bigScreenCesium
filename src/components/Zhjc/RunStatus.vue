...
<div class="runstatus">
    ...
    <div class="runstatus-content">
        <div class="runstatus-content-item" v-for="(item,index) in optionData" :key="index">
            <p class="runstatus-title"><span :style="{'background-color':color[index]}"></span><span>{{item.name}}</span></p>
             <div class="runstatus-value">{{item.value}}<span class="runstatus-lx" v-if="item.lx > 0">/{{item.lx}}</span></div>
        </div>
...
export default {
  ...
  methods: {
    ...
    bindListen(myChart, optionName = "runStatus") {
      let selectedIndex = "";
      let hoveredIndex = "";
      // 监听点击事件，实现选中效果（单选）
