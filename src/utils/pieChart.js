      ...
      tooltip: {
        ...
        formatter: (params) => {
          if (
            ...
            //离线添加
            let lx = option.series[params.seriesIndex].pieData.lx
            let str = `${params.seriesName}<br/>` +
            `<span style="display:inline-block;margin-right:5px;border-radius:10px;width:10px;height:10px;background-color:${params.color};"></span>` +
            `${bfb}%`
            if(lx !== undefined){
                str += `<br/>` +
                `<span style="display:inline-block;margin-right:5px;border-radius:10px;width:10px;height:10px;background-color:rgb(212, 84, 84);"></span>` +
                `离线：${lx}`
            }
            return (str);
          }
